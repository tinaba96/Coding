...
#define INPUTLSIZE 1000  /*入力長さ*/ 
#define FILTERWSIZE 4 /*フィルタの幅*/
#define FILTERLSIZE 26 /*フィルタの長さ*/
#define FILTERNO 1 /*フィルタの個数*/
#define POOLSIZE 13 /*プーリングサイズ*/
#define POOLOUTSIZE 1 /*プーリングの出力サイズ*/
#define MAXINPUTNO 10    /*学習データの最大個数*/ 
...
int main()
{
 ...
 while(err>LIMIT){
  ...
  ++count ;
  /*誤差の出力*/
  fprintf(stderr,"%d¥t%lf¥n",count,err) ;
 }/*学習終了*/
 ...
 /*教師データに対する出力*/
 printf("Network output¥n") ;
 printf("#¥tteacher¥toutput¥n") ;
 for(i=0;i<n_of_e;++i){
  printf("%d¥t%d¥t",i,t[i]) ;
  for(j=0;j<FILTERNO;++j){/*フィルタ毎の繰り返し*/
    /*畳み込みの計算*/
    conv(filter[j],e[i],convout) ;
    /*プーリングの計算*/
...
void showdata(double e[][INPUTWSIZE][INPUTLSIZE],int t[],int n_of_e)
{
 ...
 /*データの表示*/
 for(i=0;i<n_of_e;++i){
  printf("N=%d category=%d¥n",i,t[i]) ;
  for(j=0;j<INPUTWSIZE;++j){
   for(k=0;k<INPUTLSIZE;++k)
    printf("%.3lf ",e[i][j][k]) ;
   printf("¥n") ;
  }
  printf("¥n") ;
...
int getdata(double e[][INPUTWSIZE][INPUTLSIZE],int t[])
{
 ...
 while(scanf("%d",&t[i])!=EOF){/*教師データの読み込み*/
  ...
  while(scanf("%lf",&e[i][j][k])!=EOF){
   ++ k ;
   if(k>=INPUTLSIZE){/*次のデータ*/
    k=0 ;
    ++j ;
    if(j>=INPUTWSIZE) break ;/*入力終了*/
   }
  }
  j=0; k=0 ;
  ++i ; 
