...
for x in range(N+1):
    for y in range(N+1):
        z=N-x-y
        if z>=0 and Y==10000*x+5000*y+1000*z:
            print(x,y,z)
            exit()
print(-1,-1,-1)
...
S=input()
Sr=S.replace("eraser","").replace("erase","").replace("dreamer","").replace("dream","")
if Sr=='':
    print('YES')
else:
    print('NO')
...
for i in range(N):
    if flag==True:
        if (t[i+1]-t[i])>=(x[i+1]-x[i]+y[i+1]-y[i]) and (t[i+1]-t[i])%2==0 and (x[i+1]-x[i]+y[i+1]-y[i
            #print(t[i+1]-t[i])
            flag=True
        elif (t[i+1]-t[i])>=(x[i+1]-x[i]+y[i+1]-y[i]) and (t[i+1]-t[i])%2!=0 and (x[i+1]-x[i]+y[i+1]-y
            #print(t[i+1]-t[i])
            flag=True
        else:
